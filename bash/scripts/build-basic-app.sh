...
__find_and_replace ".*assetURL\":.*" "    \"assetURL\": \"$assetURI/$ASSET_TYPE\"," "config.json" "$buildBasicAppLogDir"
__find_and_replace ".*timeseriesURL\":.*" "    \"timeseriesURL\": \"$TIMESERIES_QUERY_URI\"" "config.json" "$buildBasicAppLogDir"
